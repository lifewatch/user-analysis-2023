...
def annonymize_input(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "email" in df.columns:  # overwritten if more specific information is available
        _mailEnds = [
            str(email).split("@")[-1] if email is not None else "NA"
            for email in df["email"]
        ]
        _split_mailsEnds = [
            mail_end.split(".") if mail_end is not None else "NA"
            for mail_end in _mailEnds
        ]

        df_["email_domain_end"] = [item_lst[-1] for item_lst in _split_mailsEnds]
        df_["email_domain_name"] = [
            "".join(item_lst[0:-1]) for item_lst in _split_mailsEnds
        ]
    ...
    if "affiliation" in df.columns:
        df_["raw_institute"] = df["affiliation"]
    ...
    for index, row in df.iterrows():
        for stand_inst, inst_list in mapping_dct.items():
            if "email_domain_name" in df.columns:
                if (
                    row["email_domain_name"] in inst_list
                    or str(row["email_domain_name"]).lower().strip() in inst_list
                ):
                    df.at[index, "stand_institute"] = stand_inst
            if "raw_institute" in df.columns:
                if (
                    row["raw_institute"] in inst_list
                    or str(row["raw_institute"]).lower().strip() in inst_list
                ):
                    df.at[index, "stand_institute"] = stand_inst
...
def add_info(df: pd.DataFrame, affil_info: pd.DataFrame) -> pd.DataFrame:
    ...
    merged_df = pd.merge(
        df, affil_info, on="stand_institute", how="left", suffixes=("_", "_info")
    )
    return merged_df
...
REFINPUT_FILEPATH = (
    PROJECTPATH / "data" / "reference_data" / "AffiliationMappingFile.csv"
)
