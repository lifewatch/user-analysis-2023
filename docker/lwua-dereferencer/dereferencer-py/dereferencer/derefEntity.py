...
def make_tasks(propertypaths):
    ...
    def helper(propertypath, task=[]):
        if isinstance(propertypath, dict):
            ...
            return helper(propertypath[property_to_search], task + [property_to_search])
    ...
    def run(self, graph, uri):
        while self.__len__() > 0:
            ...
            for task in self.tasks:
                ...
                # check if the query returned any results and if so then continue to the next task , delete the task from the subtasks
                if len(q_r) > 0:
            ...
            if self.failed_tasks == last_failed_tasks and self.successful_tasks == last_successful_tasks:
