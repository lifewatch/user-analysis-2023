...
from lwua.graph_functions import (
    ingest_data_file,
    delete_data_file,
    get_admin_graph,
)
...
class Watcher:
    def __init__(self, directory_to_watch):
        self.directory_to_watch = resolve_path(
            os.getenv(
                "GDB_DATA_FOLDER",
                os.path.join("/root/graphdb-import", directory_to_watch),
            ),
            "dotenv",
        ).absolute()
    ...
    def get_all_files(self):
        return {
            ...
            for f in Path(self.directory_to_watch).glob("**/*")
            if f.is_file()
        }
        ...
        try:
            while True:
                if self.first_loop:
                    ...
                    # try and get the graph , if this fails due to the graphdb
                    # server not being up yet, the watcher will try again in 5
                    # seconds untill it succeeds
                    get_admin = True
                    while get_admin:
                        try:
                            # get admin graph
                            ...
                            info_admin = [
                                ...
                                for g in admin_graph["results"]["bindings"]
                            ]
                            ...
                            # if filename is in the admin graph, but not in the
                            # files, delete the graph
                            info_admin_dict = {g[0]: g[1] for g in info_admin}

                            for g in info_admin:
                                if g[0] not in all_files:
                                    log.info(
                                        f"File {g[0]} has been deleted since downtime, deleting graph"
                                    )
                            ...
                            for f in all_files:
                                if f in info_admin_dict:
                                    ...
                                    if info_admin_dict[f] < all_files[f][0]:
                                        log.info(
                                            f"File {f} has been modified since downtime, updating graph"
                                        )
                                        ingest_data_file(f, True)
                                    continue
                                log.info(
                                    f"File {f} has been added since downtime, adding graph"
                                )
                                ingest_data_file(f)
