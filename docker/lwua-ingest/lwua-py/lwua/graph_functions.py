...
def update_context_admin(
    context: str, gdb: SPARQLWrapper = None, lastmod: str = None
):
...
def insert_data(graph: Graph, context: str = None, gdb: SPARQLWrapper = None):
    ...
    vars = {"context": context, "data": ntstr}
...
def ingest_graph(graph: Graph, context: str = None, replace: bool = False):
    ...
    date_string = datetime.utcfromtimestamp(c_time).isoformat()
    update_context_admin(context, gdb, date_string)
...
def fname_2_context(fname: str):
    # return named_context(f"data/{fname}") # /data prefix is not needed until
    # we have multiple data folders to ingest from
...
def get_admin_graph(gdb: SPARQLWrapper = None):
    ...
    vars = {"admin_context": admin_context()}
    query = j2rdf.build_syntax(template, **vars)
