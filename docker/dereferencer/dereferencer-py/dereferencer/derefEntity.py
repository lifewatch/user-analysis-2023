# This file will contain all the classes and functions needed to complete the dereferencing process
...
def check_metadata_folder_exists():
    ...
    if not os.path.exists(metadata_folder):
        log.info(f"metadata folder {metadata_folder} does not exist, creating it")
        ...
        if isinstance(item, dict):
            for key, values in item.items():
                if isinstance(values, list):
                    ...
                    result.append({key: {"completed": False, "children": children}})
                else:
...
class DerefUriEntity:
    def __init__(self, uri:str, propertypaths:dict):
        ...
        if not os.path.exists(METADATA_FILE):
            log.info(f"metadata file {METADATA_FILE} does not exist, creating it")
            with open(METADATA_FILE, 'w') as f:
                json.dump({}, f)
        else:
            with open(METADATA_FILE, 'r') as f:
