...
class Dereference:
    ...
    def run_dereference(self):
        ...
        # the files should be in yml or yaml format and should start with dereference
        ...
        for config_file in config_files:
            ...
            with open(config_file, 'r') as stream:
                try:
                    ...
                    # make a derefEntity for each uri in the uri_list_from_query
                    for uri in uri_list_from_query:
                        log.info(f"uri: {uri}")
                        derefEntity = DerefUriEntity(uri, config["property_paths"])
